  ...
  const systemPrompt =
    mode === "teacher"
      ? [
          ...
          theme === "food"
            ? "闽食主题要围绕泉州本地食育：认识名称、观察食材和外形、温和接受陌生食物、家园共育介绍一道家乡美食；可生成闽食小列车站点口令、美食猜猜乐线索、小小闽食播报员介绍词、泉州小厨房动作口令、区域活动步骤、家长端家庭小任务和温和表扬语，不要只写进食问题处理。"
            : "幼习宝主题要围绕生活习惯、进餐习惯、阅读表达、安全与情绪，帮助幼儿在一日生活中迁移一个小行为；可生成阅读打卡故事提问、文明进餐操口令、好习惯红绿牌题目、亲子共读任务、表扬语、奖章口令和家园同步话术。",
          "如果输入包含幼儿互动记录、gameKey、选择记录或家长反馈，请先提炼观察线索，再给出后续活动、鼓励语、家长同步建议和一个家庭小任务。",
