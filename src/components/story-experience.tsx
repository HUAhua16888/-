...
function KindWordsGame({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    if (score >= 2 && !completionReportedRef.current) {
      completionReportedRef.current = true;
      onComplete?.();
    }
  }, [onComplete, score]);
...
function GrowthArchivePanel({
  ...
  async function copyFullArchive() {
    ...
    try {
      ...
      setArchiveActionStatus("完整成长档案已复制，可以粘贴到本地文档留存。");
    } catch {
      setArchiveActionStatus("复制完整档案失败，可以改用导出文件。");
    }
  }
  ...
  function exportArchiveJson() {
    ...
    setArchiveActionStatus("成长档案已导出，可用于本机备份或转交给老师。");
  }
...
export function StoryExperience() {
  ...
  function logMiniGameCompletion(gameKey: MiniGameKey, badgeName: string) {
    updateGrowthArchive((current) => {
      ...
      return recordBadge(withGame, badgeName, miniGameThemeMap[gameKey], "mini-game");
    });
