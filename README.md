...
- 统一顶栏导航与移动端底部导航
- 教师工作台本机自动保存草稿
- 教师发布绘本与待定习惯模板：儿童端可自主选听绘本、语音或文字回应模板并形成打卡记录
- 班级快照同步：同一教师账号可上传/拉取花名册、成长记录、食谱、家园记录、教师发布内容和生成历史
- 每周食谱按日期自动进入儿童端，观察卡会根据当天食谱生成食材认识与营养提示
- 泉州小厨房支持自定义菜名、语音说菜、步骤图卡和按菜品生成的播报词
- 桌面端支持从各身份页面回首页切换身份
...
隐私与正式上线提醒：当前版本为班级小范围试用/参赛演示版本，数据主要保存在这台设备上；新增班级快照同步用于本地/服务器文件快照恢复，不是正式数据库云同步。正式上线需要补齐后端账号系统、HTTPS、密码加密存储、角色权限、日志审计、数据备份、数据删除与家长授权管理机制。

- 儿童端不设账号，只用小名牌/号数互动。
- 家长端通过幼儿姓名或号数 + 家庭绑定码查看自己孩子。
- 语音只保存识别后的文字，不保存原始语音。
- 照片/视频默认本机预览，不上传云端。
- 当前为班级试用/参赛演示版本，班级快照同步仅用于同一教师账号的文件快照恢复；正式上线需后端账号、数据库、加密、权限、审计、备份和删除机制。
